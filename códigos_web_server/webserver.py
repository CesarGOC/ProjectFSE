...
import json
import magic
from web_smart_home import ledson,ledsoff,timbre,bcd,pwm,garage,camara
...
class WebServer(BaseHTTPRequestHandler):
	...
	def _parse_post(self, json_obj):
		...
		if func:
			...
			func(json_obj['value'])
	#"""do_GET controla todas las solicitudes recibidas vía
	#GET, es decir, páginas. Por seguridad, no se analizan
	#variables que lleguen por esta vía"""
	def do_GET(self):
	...
	#"""do_POST controla todas las solicitudes recibidas ví
	#a POST, es decir, envíos de formulario. Aquí se gestionan los
	#comandos para la Raspberry Pi"""
	def do_POST(self):
...
def main():
	...
	print ("\tAtendiendo solicitudes en http://{}:{}".
		format(
		address, port))
	try:
